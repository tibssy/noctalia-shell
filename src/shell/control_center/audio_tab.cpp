...
namespace {
  ...
  public:
    ProgramVolumeRow(PipeWireService* audio, std::uint32_t id, float sliderMax, float scale,
      ...
      controlsRow->addChild(ui::slider({
          .out = &m_slider,
          .minValue = 0.0f,
          .maxValue = sliderMax,
          .step = 0.01f,
      ...
      controlsRow->addChild(ui::button({
          .out = &m_muteButton,
          .glyph = "volume-high",
...
std::unique_ptr<Flex> AudioTab::create() {
  ...
  outputVolumeCard->addChild(ui::label({
  ...
  inputVolumeCard->addChild(ui::label({
